import { assert_equals, assert_true, assert_unreached, promise_test } from "../test-utils";
...
async function handleRequest(request) {
  ...
  try {
    ...
    for (let size in sourceData) {
      for (let alg in digestedData) {
        ...
        await promise_test(function (test) {
          var promise = subtle.digest({ name: badName }, sourceData[size])
            .then(function (result) {
              ...
              // TODO: we don't throw the correct error types yet

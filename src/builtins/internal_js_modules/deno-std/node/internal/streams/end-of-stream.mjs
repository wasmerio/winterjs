...
import { AbortError, ERR_STREAM_PREMATURE_CLOSE } from "../errors";
...
import * as process from "../../_process/process";
...
function eos(stream, options, callback) {
  ...
  const readable =
    options.readable || (options.readable !== false && isReadable(stream));
  const writable =
    options.writable || (options.writable !== false && isWritable(stream));

...
} from "../internal_binding/uv";
import { assert } from "../../_util/asserts";
import { isWindows } from "../../_util/os";
import { os as osConstants } from "../internal_binding/constants";
const {
  errno: { ENOTDIR, ENOENT },
} = osConstants;
import { hideStackFrames } from "./hide_stack_frames";
import { getSystemErrorName } from "../_utils";
export { errorMap };
...
globalThis.Error.captureStackTrace =
  ...
  function (e) {
    ...
    while (lines.length > 0 && lines[0].includes("$winterjs_internal$")) {
      lines.splice(0, 1);
    }
 ...
 * @returns
 */
const genericNodeError = hideStackFrames(function genericNodeError(
  message,
  errorProperties
) {
  // eslint-disable-next-line no-restricted-syntax

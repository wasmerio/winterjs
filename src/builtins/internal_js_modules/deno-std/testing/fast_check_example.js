...
import { groupBy } from "../collections/group_by";
import {
  assert,
  assertEquals,
} from "https://deno.land/std@0.90.0/testing/asserts";
/********************** contains() *************************************/
const contains = (text, pattern) => text.indexOf(pattern) >= 0;
Deno.test(
  "Can use fast-check to property test contains function",
  async (t) => {
...
Deno.test(
  "Can use fast-check to property test decomposePrime function",
  async (t) => {
    await t.step(
...
const MaxN = 1000;
Deno.test(
  "Can use fast-check to property test fibronacci function",
  async (t) => {
    await t.step(
    ...
    () => {
      fc.assert(
        fc.property(fc.array(fc.integer()), (data) => {
          ...
          const keyCreatorFn = (item) => item.toString();
          assertEquals(
...
Deno.test(
  "Can use fast-check to property test lowercaseString function",
  async (t) => {
    await t.step("string after lower case should be of same length", () => {
      fc.assert(
        fc.property(
          fc.string(),
          (text) => text.length === lowercaseString(text).length
        )
      );
    });
  }
...
function absoluteAddition(a, b) {
  return Math.abs(a) + Math.abs(b);
}
Deno.test(
  "Can use fast-check to property test add and absoluteAddition functions",
  async (t) => {
    await t.step(
      ...
      () => {
        fc.assert(
          fc.property(
            fc.integer(-99, 99),
            fc.integer(-99, 99),
            (a, b) => add(a, b) <= absoluteAddition(a, b)
          )
        );
      }
    );
